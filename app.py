...
from flask import Flask, render_template, request, redirect, jsonify, url_for, session, send_from_directory, abort, g
import psycopg2
from psycopg2.extras import RealDictCursor
from db_config import get_sqlite_connection, get_postgres_connection, close_connections, init_connection_pool, shutdown_pool
from logger_config import db_logger, error_logger, perf_logger, access_logger, log_query_time, log_error, log_access
...
# Inicializa o pool de conexões PostgreSQL quando a aplicação inicia
...
# Encerra o pool de conexões quando a aplicação é encerrada
@app.teardown_appcontext
def close_db_pool(error):
    if error:
        print(f"Erro ao encerrar aplicação: {error}")
    shutdown_pool()
...
# API para gerenciar números específicos (excluir/atualizar)
...
# API para gerenciar números de WhatsApp - PostgreSQL
@app.route('/api/pg/numbers', methods=['GET', 'POST'])
@app.route('/api/pg/numbers/<int:number_id>', methods=['DELETE', 'PUT'])
@login_required
@log_query_time
def manage_numbers_pg(number_id=None):
    """Endpoint para gerenciar números usando PostgreSQL"""
    user_id = session.get('user_id')
    pg_conn = get_postgres_connection()
    
    if not pg_conn:
        # Fallback para SQLite
        print("PostgreSQL indisponível. Fallback para SQLite.")
    ...
    try:
        if request.method == 'POST':
            ...
            with pg_conn.cursor() as cursor:
                # Verificar se o número já existe para este usuário
                cursor.execute(
                    'SELECT * FROM whatsapp_numbers WHERE phone_number = %s AND user_id = %s', 
                ...
                # Inserir o novo número
                cursor.execute(
                    'INSERT INTO whatsapp_numbers (user_id, phone_number, description) VALUES (%s, %s, %s) RETURNING id', 
                    (user_id, validated_number, description)
                )
        ...
        print(f"Erro ao gerenciar números com PostgreSQL: {e}")
        if pg_conn:
            pg_conn.close()
        # Fallback para SQLite
        return manage_numbers()

# API para gerenciar números específicos (excluir/atualizar) - PostgreSQL
@app.route('/api/pg/numbers/<int:number_id>', methods=['DELETE', 'PUT'])
@login_required
def manage_number_by_id_pg(number_id):
    """Endpoint para gerenciar número específico usando PostgreSQL"""
    user_id = session.get('user_id')
    pg_conn = get_postgres_connection()
    ...
    try:
        with pg_conn.cursor() as cursor:
            # Verificar se o número pertence ao usuário atual
            cursor.execute(
                'SELECT * FROM whatsapp_numbers WHERE id = %s AND user_id = %s', 
                (number_id, user_id)
            )
            ...
            if request.method == 'DELETE':
                ...
                if request.is_json:
                    ...
                    if update_fields:
                        params.append(number_id)
                        cursor.execute(
                            f'UPDATE whatsapp_numbers SET {", ".join(update_fields)} WHERE id = %s', 
                            params
                        )
                        pg_conn.commit()
                        pg_conn.close()
        ...
        print(f"Erro ao gerenciar número específico com PostgreSQL: {e}")
        if pg_conn:
            pg_conn.close()
        # Fallback para SQLite
        return delete_number(number_id)

# API para gerenciar links personalizados usando PostgreSQL
@app.route('/api/pg/links', methods=['GET', 'POST'])
@app.route('/api/pg/links/<int:link_id>', methods=['DELETE', 'PUT'])
@login_required
@log_query_time
def manage_links_pg(link_id=None):
    """Endpoint para gerenciar links personalizados com PostgreSQL"""
    user_id = session.get('user_id')
    pg_conn = get_postgres_connection()
    ...
    try:
        if request.method == 'POST':
            ...
            with pg_conn.cursor() as cursor:
                ...
                # Determinar o próximo prefixo disponível para este link
                cursor.execute('''
                    SELECT COALESCE(MAX(prefix), 0) + 1 as next_prefix
                    FROM custom_links
                    WHERE user_id = %s AND link_name = %s
                ''', (user_id, link_name))
                
                prefix_result = cursor.fetchone()
                prefix = prefix_result['next_prefix'] if prefix_result else 1
                
                # Inserir o novo link com o prefixo calculado
                cursor.execute(
                    'INSERT INTO custom_links (user_id, link_name, custom_message, prefix) VALUES (%s, %s, %s, %s) RETURNING id',
                    (user_id, link_name, custom_message, prefix)
                )
                
                pg_conn.commit()
                
                # Retornar também o prefixo e formatos de URL
                result = {
                    'success': True, 
                    'message': 'Link adicionado com sucesso',
                    'prefix': prefix,
        ...
        print(f"Erro ao gerenciar links com PostgreSQL: {e}")
        if pg_conn:
            pg_conn.close()
        # Fallback para SQLite
        return manage_links()

# API para excluir links personalizados usando PostgreSQL
@app.route('/api/pg/links/<int:link_id>', methods=['DELETE'])
@login_required
def delete_link_pg(link_id):
    """Endpoint para excluir link personalizado usando PostgreSQL"""
    user_id = session.get('user_id')
    pg_conn = get_postgres_connection()
    ...
    try:
        with pg_conn.cursor() as cursor:
            # Verificar se o link pertence ao usuário atual
            cursor.execute(
                'SELECT * FROM custom_links WHERE id = %s AND user_id = %s', 
                (link_id, user_id)
            )
        ...
        print(f"Erro ao excluir link com PostgreSQL: {e}")
        if pg_conn:
            pg_conn.close()
        # Fallback para SQLite
        return delete_link(link_id)

# API para atualizar links personalizados usando PostgreSQL
@app.route('/api/pg/links/<int:link_id>', methods=['PUT'])
@login_required
def update_link_pg(link_id):
    """Endpoint para atualizar link personalizado usando PostgreSQL"""
    user_id = session.get('user_id')
    data = request.json
    pg_conn = get_postgres_connection()
    ...
    try:
        with pg_conn.cursor() as cursor:
            ...
            if 'custom_message' in data:
                cursor.execute(
                    'UPDATE custom_links SET custom_message = %s WHERE id = %s', 
                    (data['custom_message'], link_id)
                )
            
            if 'is_active' in data:
                cursor.execute(
                    'UPDATE custom_links SET is_active = %s WHERE id = %s', 
                    (1 if data['is_active'] else 0, link_id)
                )
        ...
        print(f"Erro ao atualizar link com PostgreSQL: {e}")
        if pg_conn:
            pg_conn.close()
        # Fallback para SQLite
        return update_link(link_id)
...
@app.route('/r/<path:link_name>')
@log_query_time
def redirect_whatsapp_pg(link_name):
    """Endpoint principal de redirecionamento para WhatsApp"""
    try:
        # Tentamos usar o PostgreSQL primeiro
        return redirect_whatsapp_func_postgres(link_name)
    except Exception as e:
        print(f"Erro ao usar PostgreSQL para redirecionamento, usando SQLite: {e}")
        # Em caso de falha, usamos SQLite como fallback
        return redirect_whatsapp_func(link_name)

# Rota de redirecionamento com prefixo
@app.route('/p/<int:prefix>/<path:link_name>')
def redirect_whatsapp_prefix_pg(prefix, link_name):
    """Endpoint de redirecionamento com prefixo (para múltiplos usuários com mesmo nome de link)"""
    try:
        # Tentamos usar o PostgreSQL primeiro
        return redirect_whatsapp_func_postgres(link_name, prefix)
    except Exception as e:
        print(f"Erro ao usar PostgreSQL para redirecionamento com prefixo, usando SQLite: {e}")
        # Em caso de falha, usamos SQLite como fallback
        return redirect_whatsapp_func(link_name, prefix)

# Rota de redirecionamento com ID do usuário
@app.route('/u/<int:user_id>/<path:link_name>')
def redirect_whatsapp_user_pg(user_id, link_name):
    """Endpoint de redirecionamento com ID do usuário"""
    try:
        # Tentamos usar o PostgreSQL primeiro
        return redirect_whatsapp_func_postgres(link_name, None, user_id)
    except Exception as e:
        print(f"Erro ao usar PostgreSQL para redirecionamento com usuário, usando SQLite: {e}")
        # Em caso de falha, usamos SQLite como fallback
        return redirect_whatsapp_func(link_name, None, user_id)
...
def redirect_whatsapp_func(link_name, prefix=None, user_id=None):
    ...
    try:
        ...
        # Tentar fallback para PostgreSQL
        try:
            print("Fallback para PostgreSQL após erro no SQLite.")
            return redirect_whatsapp_func_postgres(link_name, prefix, user_id)
        except Exception as e2:
            print(f"Erro também no fallback PostgreSQL: {str(e2)}")
...
def get_stats_summary():
    ...
    redirect_params = params.copy()
    if not start_date:
        redirect_params.append(datetime.now().strftime('%Y-%m-%d'))  # Data atual como limite superior
    ...
    with get_db_connection() as conn:
        ...
        total_clicks = conn.execute(total_clicks_query, params).fetchone()['total']
        ...
        if start_date and end_date:
            ...
            daily_average_query = f'''
                ...
                WHERE {link_condition} {date_condition}
            '''
            daily_params = params + [end_date, start_date]
...
def get_stats_by_number():
    ...
    with get_db_connection() as conn:
        # Verificar contagem total considerando a agregação por minuto
        count_query = f'''
            SELECT COUNT(*) as total
            FROM redirect_logs rl
            JOIN custom_links cl ON rl.link_id = cl.id
...
# Função para obter condições de filtro padronizadas para PostgreSQL
def get_standard_filter_conditions_postgres(user_id, link_id, start_date, end_date):
    """
    Cria condições SQL e parâmetros padronizados para filtros no PostgreSQL, 
    garantindo consistência entre diferentes consultas.
    """
    # Construir a condição de link_id
    ...
    date_condition = ""
    if start_date:
        date_condition += " AND DATE(rl.redirect_time) >= DATE(%s)"
        params.append(start_date)
    if end_date:
        date_condition += " AND DATE(rl.redirect_time) <= DATE(%s)"
        params.append(end_date)
...
# API para obter estatísticas por link usando PostgreSQL
@app.route('/api/stats/pg/by-link', methods=['GET'])
@login_required
def get_stats_by_link_postgres():
    user_id = session.get('user_id')
    start_date = request.args.get('start_date')
    end_date = request.args.get('end_date')
    link_id = request.args.get('link_id', 'all')
    
    print(f"[PostgreSQL] Query de estatísticas por link - user_id: {user_id}, link_id: {link_id}, start_date: {start_date}, end_date: {end_date}")
    
    try:
        pg_conn = get_postgres_connection()
        if not pg_conn:
            # Fallback para SQLite se o PostgreSQL não estiver disponível
            print("PostgreSQL indisponível para estatísticas. Fallback para SQLite.")
            return get_stats_by_link()
        
        # Obter condições e parâmetros de forma padronizada
        link_condition, date_condition, params = get_standard_filter_conditions_postgres(user_id, link_id, start_date, end_date)
        ...
        with pg_conn.cursor() as cursor:
            ...
            # Montar resultado com datas contínuas
            daily_result = []
            if daily_stats:
                # Converter para dicionário para acesso mais fácil
                date_dict = {row["date"].strftime('%Y-%m-%d'): row["count"] for row in daily_stats}
                
                # Garantir que temos todas as datas no intervalo
                if start_date and end_date:
                    ...
                    # Se não temos intervalo de datas, apenas usar o que temos
            ...
            country_query = f'''
                SELECT 
                    COALESCE(rl.country, 'Desconhecido') as country,
                    COUNT(*) as count
                FROM redirect_logs rl
                JOIN custom_links cl ON rl.link_id = cl.id
                WHERE {link_condition} {date_condition}
                GROUP BY COALESCE(rl.country, 'Desconhecido')
                ORDER BY count DESC
                LIMIT 10
            '''
            ...
            hourly_query = f'''
                SELECT 
                    EXTRACT(HOUR FROM rl.redirect_time) as hour,
                    COUNT(*) as count
                FROM redirect_logs rl
                JOIN custom_links cl ON rl.link_id = cl.id
                WHERE {link_condition} {date_condition}
                GROUP BY EXTRACT(HOUR FROM rl.redirect_time)
                ORDER BY hour
            '''
            ...
            # Garantir que temos todas as 24 horas no resultado
            hour_dict = {int(row["hour"]): row["count"] for row in hourly_stats}
            hourly_result = []
            
            for i in range(24):
                hourly_result.append({
                    "hour": f"{i:02d}",
                    "count": hour_dict.get(i, 0)
                })
        ...
        print(f"Erro ao obter estatísticas PostgreSQL: {e}")
        if 'pg_conn' in locals() and pg_conn:
            pg_conn.close()
        # Fallback para SQLite
        return get_stats_by_link()

# API para obter estatísticas por chip (número) usando PostgreSQL
@app.route('/api/stats/pg/by-number', methods=['GET'])
@login_required
def get_stats_by_number_postgres():
    user_id = session.get('user_id')
    start_date = request.args.get('start_date')
    end_date = request.args.get('end_date')
    link_id = request.args.get('link_id', 'all')
    
    print(f"[PostgreSQL] Query de estatísticas por chip - user_id: {user_id}, link_id: {link_id}, start_date: {start_date}, end_date: {end_date}")
    ...
    try:
        ...
        with pg_conn.cursor() as cursor:
            ...
            print(f"[PostgreSQL] Total de acessos para filtros atuais (estatísticas por chip): {total_count}")
            
            # Obter todos os números do usuário
            cursor.execute('SELECT id, phone_number, description FROM whatsapp_numbers WHERE user_id = %s', [user_id])
            numbers = cursor.fetchall()
            result = []
            total_por_chip = 0
            ...
            for number in numbers:
                ...
                precise_query = f'''
                    ...
                    FROM redirect_logs rl
                    JOIN custom_links cl ON rl.link_id = cl.id
                    WHERE rl.number_id = %s AND {link_condition} {date_condition}
                '''
                ...
                cursor.execute(precise_query, num_params)
            ...
            orphan_query = f'''
                ...
                JOIN custom_links cl ON rl.link_id = cl.id
                WHERE {link_condition} {date_condition}
                AND rl.number_id NOT IN (SELECT id FROM whatsapp_numbers WHERE user_id = %s)
            '''
            ...
            if orphan_count > 0:
                ...
                total_por_chip += orphan_count
                print(f"[PostgreSQL] Encontrados {orphan_count} redirecionamentos para números excluídos")
            
            # Verificar discrepâncias na contagem
            print(f"[PostgreSQL] Total contado por números: {total_por_chip}, Total geral: {total_count}")
        ...
        print(f"Erro ao obter estatísticas PostgreSQL por número: {e}")
        if 'pg_conn' in locals() and pg_conn:
            pg_conn.close()
        # Fallback para SQLite
        return get_stats_by_number()
...
# Função que contém a lógica de redirecionamento usando PostgreSQL
def redirect_whatsapp_func_postgres(link_name, prefix=None, user_id=None):
    redirect_start_time = datetime.now()
    ...
    # Logar o IP capturado para debug
    print(f"[PostgreSQL] IP capturado para redirecionamento: {client_ip}, Link: {link_name}, Prefix: {prefix}, User ID: {user_id}")
    ...
    try:
        ...
        if not pg_conn:
            # Fallback para SQLite se o PostgreSQL não estiver disponível
            print("PostgreSQL indisponível. Fallback para SQLite.")
            return redirect_whatsapp_func(link_name, prefix, user_id)
        ...
        with pg_conn.cursor() as cursor:
            ...
            query = '''
                ...
                FROM custom_links cl
                JOIN users u ON cl.user_id = u.id
                WHERE cl.link_name = %s AND cl.is_active = 1
            '''
            params = [link_name]
            ...
            # Incrementar contador de cliques
            cursor.execute('UPDATE custom_links SET click_count = click_count + 1 WHERE id = %s', (link['id'],))
            ...
            # Selecionar um número usando algoritmo balanceado
            selected_number = select_balanced_number_postgres(pg_conn, numbers, link['id'])
            phone_number = selected_number['phone_number']
            
            # Garantir que o número esteja no formato internacional
            phone_number = format_phone_number(phone_number)
            
            # Registrar o log de redirecionamento usando a função PostgreSQL
            log_id = record_click_postgres(link['id'], selected_number['id'], client_ip, user_agent)
        ...
        # Retornar uma página de erro genérica
        print(f"Erro no redirecionamento PostgreSQL: {str(e)}")
        redirect_time = (datetime.now() - redirect_start_time).total_seconds()
        
        if 'pg_conn' in locals() and pg_conn:
            pg_conn.close()
            
        # Tentar fallback para SQLite
        try:
            print("Fallback para SQLite após erro no PostgreSQL.")
            return redirect_whatsapp_func(link_name, prefix, user_id)
        except Exception as e2:
            print(f"Erro também no fallback SQLite: {str(e2)}")
            return render_template('index.html', error='Ocorreu um erro ao processar seu redirecionamento. Tente novamente.')

# Função para registrar cliques em links usando PostgreSQL
def record_click_postgres(link_id, number_id, ip_address=None, user_agent=None):
    """
    Registra um clique em um link usando PostgreSQL para melhor performance
    
    Args:
        link_id: ID do link acessado
        number_id: ID do número para o qual redirecionou
    ...
    try:
        ...
        if not pg_conn:
            # Fallback para SQLite se o PostgreSQL não estiver disponível
            print("Fallback para SQLite: PostgreSQL não disponível")
            return record_click_sqlite(link_id, number_id, ip_address, user_agent)
        
        with pg_conn.cursor() as cursor:
            # Inserir no log de redirecionamentos do PostgreSQL
            cursor.execute('''
                INSERT INTO redirect_logs 
                (link_id, number_id, redirect_time, ip_address, user_agent)
                VALUES (%s, %s, %s, %s, %s)
                RETURNING id
            ''', (
                link_id, number_id, datetime.now(), 
            ...
            cursor.execute('''
                ...
                SET redirect_count = redirect_count + 1,
                    last_used = %s
                WHERE id = %s
            ''', (datetime.now(), number_id))
            
            pg_conn.commit()
            close_connections(pg_conn=pg_conn)
            return log_id
            
    except Exception as e:
        print(f"Erro ao registrar clique no PostgreSQL: {e}")
        if pg_conn:
            pg_conn.rollback()
            close_connections(pg_conn=pg_conn)
        # Fallback para SQLite
...
# Função original para registrar cliques (fallback)
def record_click_sqlite(link_id, number_id, ip_address=None, user_agent=None):
    """Registra um clique usando SQLite (fallback)"""
    conn = get_db()
    try:
        # Inserir no log de redirecionamentos
        cursor = conn.execute('''
            INSERT INTO redirect_logs 
            (link_id, number_id, redirect_time, ip_address, user_agent)
            VALUES (?, ?, ?, ?, ?)
        ...
        print(f"Erro ao registrar clique no SQLite: {e}")
        conn.rollback()
        return None

# Função para selecionar um número de forma balanceada usando PostgreSQL
def select_balanced_number_postgres(pg_conn, numbers, link_id):
    """
    Seleciona um número de WhatsApp de forma balanceada usando PostgreSQL
    
    Args:
        pg_conn: Conexão com o PostgreSQL
        numbers: Lista de números disponíveis
        link_id: ID do link para considerar estatísticas específicas
    """
    try:
        # Verificar se há apenas um número disponível
        if len(numbers) == 1:
        ...
        with pg_conn.cursor() as cursor:
            for num in numbers:
                ...
                use_data = cursor.fetchone()['count']
            ...
            if novos_numeros and numeros_existentes:
                ...
                if random.random() < prob_escolher_novo:
                    ...
                    # Atualizar timestamp de último uso
                    cursor.execute('UPDATE whatsapp_numbers SET last_used = %s WHERE id = %s', 
                                (datetime.now(), selected_number['id']))
                    pg_conn.commit()
            ...
            for num in numeros_para_selecao:
                ...
                cursor.execute('''
                    ...
                    FROM redirect_logs 
                    WHERE number_id = %s 
                    AND redirect_time >= NOW() - INTERVAL '1 day'
                ''', (num['id'],))
                ...
                if link_id:
                    cursor.execute('''
                        SELECT COUNT(*) as count 
                        FROM redirect_logs 
                        WHERE number_id = %s AND link_id = %s 
                        AND redirect_time >= NOW() - INTERVAL '1 day'
                    ''', (num['id'], link_id))
        ...
        print(f"Erro ao selecionar número balanceado no PostgreSQL: {e}")
        # Fallback para seleção aleatória simples
        selected_number = random.choice(numbers)
        return selected_number
